...
export const JournalService = {
  ...
  getDailyJournalCount: (
    ...
    const isTodayJournal = lastJournalCountDate === today
    return isTodayJournal
      ? {
          lastJournalCountDate: today,
          dailyJournalCount: dailyJournalCount + 1,
        }
      : {
          lastJournalCountDate,
          dailyJournalCount: 0,
  ...
  getJournals: (
    ...
    if (splitDate?.[2]) {
      return JournalService.getJournalsByDate(store, date as ISODateString)
    }
    return JournalService.getJournalsByMonth(store, date as ISOMonthString)
  },
