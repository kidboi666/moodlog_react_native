...
export const useUser = create<UserStore>((set, get) => ({
  ...
  registerUser: async userName => {
    try {
      ...
      const newUser = await User.saveNewUser(newUserInfo)
  ...
  updateDaysSinceSignup: async () => {
    try {
      ...
      const newDaysSinceSignup = await User.saveDaysSinceSignup(
        get().userInfo,
        firstLaunchDate,
      )
  ...
  onUserInfoChange: async updatedUserInfo => {
    try {
      set({ isLoading: true, error: null })
      const newUserInfo = await User.saveUser(get().userInfo, updatedUserInfo)
  ...
  loadUserData: async () => {
    try {
      set({ isLoading: true, error: null })
      const savedUserData = await User.loadUser()
